...
function getInOutData() {
    paramDate = (document.getElementById("myDate").value);
    console.log("Graph Date is ", paramDate)
...
function callbackInOutData(response) {
    ...
    for (i = 0; i < docs.length; i++){
        if (rcnt == 0){
            // endTime = new Date(docs[i].Visit_in_date_time).toISOString();
            endTime = new Date(docs[i].Visit_in_date_time)
            // endTime = docTime.toISOString();
            rcnt ++;
        } 

        if ((new Date(docs[i].Visit_in_date_time)) >= endTime) {
            x_dt.push (endTime); //.toLocaleString());
            y_cnt.push (sum);
            sum = docs[i].In_out_ind;
            endTime = new Date(endTime.getTime() + minutes*60000);
        } 
        else {
            sum = sum + docs[i].In_out_ind;
        }
        if (x_dt.length == 0 ){
            x_dt.push (endTime); //.toLocaleString());
            y_cnt.push (sum);
